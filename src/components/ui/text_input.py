...
class TextInput(Label):
    def __init__(self, font, text="", size=48,
                 max_text=50, width=400,
                 color=(255, 255, 255),
                on_change=None):
    ...
    def text_input(self, text):
        ...
        self.text += text
        # Check if the length of the text exceed the max length
        if len(self.text) > self.max_text:
            # Slice the text till max length [begining:end]
            self.text = self.text[:self.max_text]
    ...
    def draw(self, screen):
        ...
        if self.blink_step and self.active:
            screen.blit(self.cursor,
                        (self.entity.x + self.surface.get_width() + padding,
