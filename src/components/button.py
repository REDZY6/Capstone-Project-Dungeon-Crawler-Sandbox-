...
class Button:
    def __init__(self, on, click_area=pygame.Rect(0, 0, 48, 48)):
    ...
    def update(self):
        ...
        # Increase the number to do more mouse button
        if is_mouse_just_pressed(1): # first button
            if x <= mouse_pos[0] <= x + self.click_area.width and \
                y <= mouse_pos[1] <= y + self.click_area.height:
                self.on()

def create_simple_label_button(on, font, text, x=0, y=0, size=48, color=(255, 255, 255)):
